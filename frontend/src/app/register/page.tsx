...
import { ArrowRight, Eye, EyeOff, Loader2, Sparkles, Check } from 'lucide-react'
...
function LuxInput({
  ...
  return (
    <div>
      <label htmlFor={id} className="luxury-label mb-2 block">{label}</label>
      <div
        className="relative overflow-hidden rounded-xl border transition-all duration-300"
        style={{
          borderColor: focused ? 'var(--accent-gold)' : 'var(--border)',
          boxShadow: focused ? '0 0 0 3px rgba(var(--accent-gold-rgb), 0.12)' : 'none',
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        toast.success('Account created! Welcome to FashionForge.')
  ...
  const perks = [
    'Access exclusive member-only drops',
    'Curated picks based on your style',
    'One-tap checkout & order tracking',
    'Easy returns, always free',
  ]
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <motion.div
          ...
          className="absolute -top-40 -right-32 h-[520px] w-[520px] rounded-full opacity-20"
          style={{ background: 'radial-gradient(circle, rgba(var(--accent-gold-rgb), 0.5), transparent 70%)' }}
        />
        <motion.div
          animate={{ x: [0, -28, 0], y: [0, 32, 0] }}
          transition={{ duration: 24, repeat: Infinity, ease: 'linear' }}
          className="absolute -bottom-40 -left-24 h-[600px] w-[600px] rounded-full opacity-15"
      ...
      <motion.div
        ...
        className="relative z-10 w-full max-w-[1080px] overflow-hidden rounded-3xl border"
        style={{
        ...
        <div className="grid md:grid-cols-[1fr_1.15fr]">
          ...
          >
            ...
            <div className="absolute inset-0">
              ...
              <div
                ...
                style={{ background: 'linear-gradient(to bottom, var(--bg-elevated) 0%, transparent 45%, var(--bg-elevated) 100%)' }}
              />
            </div>

            <div className="relative z-10">
              <motion.span variants={fadeUp} custom={0} className="luxury-label flex items-center gap-2">
              ...
              <motion.h2 variants={fadeUp} custom={1} className="mt-6 text-5xl lg:text-6xl luxury-heading">
                Join the<br />
                <span className="italic font-light" style={{ color: 'var(--accent-gold)' }}>Collective.</span>
              </motion.h2>

              <motion.p variants={fadeUp} custom={2} className="mt-5 luxury-body text-sm max-w-sm">
                Build your style profile, unlock curated picks, and shop faster with a personalised account.
              </motion.p>
            </div>

            {/* Perk list */}
            <motion.ul variants={stagger} className="relative z-10 mt-10 space-y-3">
              {perks.map((perk, i) => (
                <motion.li
                  key={perk}
                  variants={fadeUp}
                  ...
                  style={{
                    ...
                    borderColor: 'rgba(var(--accent-gold-rgb), 0.18)',
                    backdropFilter: 'blur(12px)',
                  }}
                >
                  <span
                    className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full"
                    style={{ background: 'rgba(var(--accent-gold-rgb), 0.18)', color: 'var(--accent-gold)' }}
                  >
                    <Check size={11} strokeWidth={3} />
                  </span>
                  <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{perk}</p>
                </motion.li>
              ))}
            </motion.ul>
          </motion.div>
          ...
          >
            ...
            <motion.div variants={fadeUp} custom={0} className="mb-7">
              <h1 className="text-4xl sm:text-5xl luxury-heading">Create Account</h1>
            ...
            <motion.div variants={fadeUp} custom={2} className="mb-6 flex items-center gap-4">
              ...
              <span className="text-[11px] uppercase tracking-widest font-medium" style={{ color: 'var(--text-muted)' }}>or</span>
              <div className="flex-1 h-px" style={{ background: 'var(--border)' }} />
            </motion.div>
            ...
            >
                    ...
                    <div className="flex flex-1 gap-1">
                      {[1, 2, 3, 4].map((n) => (
                        <div
                          ...
                          style={{
                            background: n <= strength.score ? strength.color : 'var(--border-strong)',
                          }}
                        />
                      ))}
                    </div>
              ...
              <motion.p variants={fadeUp} custom={4} className="pt-1 text-center text-[11px] leading-6" style={{ color: 'var(--text-muted)' }}>
